...
def adam_updates(params, cost_or_grads, lr=0.001, mom1=0.9, mom2=0.999):
    ...
    return tf.compat.v1.group(*updates)
...
def dense(x, num_units, nonlinearity=None, init_scale=1., counters={},init=False, ema=None, train_scale=True, init_w=tf.random_normal_initializer(0, 0.05),**kwargs):
    ...
    with tf.compat.v1.variable_scope(name):
        if init:
            ...
            x_init = tf.compat.v1.matmul(x, V_norm)
            ...
            x_init = tf.compat.v1.reshape(scale_init,[1,num_units])*(x_init-tf.reshape(m_init,[1,num_units]))
...
def conv2d(x, num_filters, filter_size=[3,3], stride=[1,1], pad='SAME', nonlinearity=None, init_scale=1., counters={}, init=False, ema=None, **kwargs):
    ...
    with tf.compat.v1.variable_scope(name):
        if init:
            ...
            # print(b)
            x_init = tf.compat.v1.reshape(scale_init,[1,1,1,num_filters])*(x_init-tf.reshape(m_init,[1,1,1,num_filters]))
            if nonlinearity is not None:
                x_init = nonlinearity(x_init)
            return x_init
            ...
            # use weight normalization (Salimans & Kingma, 2016)
            W = tf.compat.v1.reshape(g,[1,1,1,num_filters])*tf.nn.l2_normalize(V,[0,1,2])
...
def deconv2d(x, num_filters, filter_size=[3,3], stride=[1,1], pad='SAME', nonlinearity=None, init_scale=1., counters={}, init=False, ema=None, **kwargs):
    ...
    name = get_name('deconv2d', counters_copy)
    xs = int_shape(x)
    if pad=='SAME':
    ...
    with tf.compat.v1.variable_scope(name):
        if init:
            ...
            W = tf.compat.v1.reshape(g,[1,1,num_filters,1])*tf.nn.l2_normalize(V,[0,1,3])
...
def nin(x, num_units, **kwargs):
    ...
    s = int_shape(x)
    x = tf.compat.v1.reshape(x, [np.prod(s[:-1]),s[-1]])
    x = dense(x, num_units, **kwargs)
    return tf.compat.v1.reshape(x, s[:-1]+[num_units])
...
def gated_resnet(x, a=None, h=None, nonlinearity=concat_elu, conv=conv2d, init=False, counters={}, ema=None, dropout_p=0., **kwargs):
    ...
    if h is not None:
        ...
        c2 += tf.compat.v1.reshape(tf.matmul(h, hw), [xs[0], 1, 1, 2 * num_filters])

    a, b = tf.compat.v1.split(3, 2, c2)
...
def down_shift(x):
    xs = int_shape(x)
    return tf.compat.v1.concat(1,[tf.zeros([xs[0],1,xs[2],xs[3]]), x[:,:xs[1]-1,:,:]])

def right_shift(x):
    xs = int_shape(x)
    return tf.compat.v1.concat(2,[tf.zeros([xs[0],xs[1],1,xs[3]]), x[:,:,:xs[2]-1,:]])

#@add_arg_scope
@tf.function
def down_shifted_conv2d(x, num_filters, filter_size=[2,3], stride=[1,1], **kwargs):
    x = tf.compat.v1.pad(x, [[0,0],[filter_size[0]-1,0], [int((filter_size[1]-1)/2),int((filter_size[1]-1)/2)],[0,0]])
...
def down_right_shifted_conv2d(x, num_filters, filter_size=[2,2], stride=[1,1], **kwargs):
    x = tf.compat.v1.pad(x, [[0,0],[filter_size[0]-1, 0], [filter_size[1]-1, 0],[0,0]])
    return conv2d(x, num_filters, filter_size=filter_size, pad='VALID', stride=stride, **kwargs)
